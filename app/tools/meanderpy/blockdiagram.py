...
def create_block_diagram(strat,dx,ve,xoffset,yoffset,scale,ci,strat_switch,contour_switch,bottom,topo_min,topo_max,vmin=None,vmax=None):
    ...
    if contour_switch == 1:
        ...
        n_contours = 10
        contours = np.linspace(z_min, z_max, n_contours)
        mlab.contour_surf(X1,Y1,z,contours=contours.tolist(),warp_scale=ve,color=(0,0,0),line_width=1.0)
...
def create_exploded_view(strat, facies, topo, h, nx, ny, gap, dx, ve, scale, strat_switch, layers_switch, contour_switch, color_mode, colors, line_thickness, bottom, export, vmin=None, vmax=None):
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            xoffset = (y1+j*gap)*dx
            yoffset = (x1+i*gap)*dx
            create_block_diagram(strat[y1:y2,x1:x2,:],dx,ve,xoffset,yoffset,scale,5.0,strat_switch,contour_switch,bottom,topo_min,topo_max,vmin,vmax)
